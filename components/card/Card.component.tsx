import React, { useEffect } from "react";
import { CardComponentPropsInterface } from "../../interface/blog-interface";
import Link from "next/link";

function CardComponent(props: CardComponentPropsInterface) {
  useEffect(() => {
    // console.log("props", props.cardData);
  }, [props]);

  return (
    <div>
      <Link href={`/blog/${props.cardData.id}`}>
        <div className="max-w-sm rounded overflow-hidden shadow-lg">
          <img
            className="w-full"
            src={
              "https://unsplash.com/photos/UZOpP-YHe9Q/download?ixid=MnwxMjA3fDB8MXxzZWFyY2h8NXx8YXVyb3JhfGVufDB8fHx8MTY4MTIxNDM1MA&force=true"
            }
            alt="Sunset in the mountains"
          />
          <div className="px-6 py-4">
            <div className="font-bold text-xl mb-2">{props.cardData.title}</div>
            <p className="text-gray-700 text-base">
              {props.cardData.description}
            </p>
          </div>
          <div className="px-6 pt-4 pb-2">
            <span className="inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2">
              @{props.cardData.users.username}
            </span>
            <span className="inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2">
              {props.cardData.users.email}
            </span>
            <span className="inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2">
              #winter
            </span>
          </div>
        </div>
      </Link>
    </div>
  );
}

export default CardComponent;
